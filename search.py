...
_CSI = _ESC + "["
_RESET = _CSI + "m"
_BG_BAR = _CSI + "48;2;24;24;37m"
_FG_BAR = _CSI + "38;2;205;214;244m"
_FG_MATCH = _CSI + "48;2;166;227;161;38;2;30;30;46m"
_FG_CUR = _CSI + "48;2;249;226;175;38;2;30;30;46m"
_FG_NOMATCH = _CSI + "38;2;243;139;168m"
_FG_BOLD = _CSI + "1m"
...
class SearchState:
    ...
    @property
    def yank_text(self) -> str:
        return "YANK" if self.yank_mode else ""
...
class SearchUI:
    ...
    def render_search_bar(self, state: SearchState) -> None:
        ...
        if state.yank_mode:
            suffix = f"{state.counter_text} {state.mode_flag} {state.case_flag} {state.yank_text}"
...
class SearchKitten:
    ...
    def _yank_match(self) -> None:
        """Yank the entire line containing the current match to clipboard and exit."""
        line_num = self.state.current_line
        if line_num is None or line_num < 0 or line_num >= len(self.lines):
            return
        # Strip ANSI codes from the line
        raw_text = self.lines[line_num]
        clean_text = re.sub(r"\x1b\[[^m]*m", "", raw_text)
        # Set yank mode so main() returns the yanked text
        self.state.yank_mode = True
        self._yanked_text = clean_text
        self.running = False
...
def main(args: List[str]) -> str:
    ...
    if result_line is None:
        return ""
    # Check if we're in yank mode - handle clipboard directly here
    if kitten.state.yank_mode:
        yanked_text = kitten._yanked_text
        # Clean tree view characters and icons
        yanked_text = re.sub(r"^[\s│├└─┌┐┘┬┼┤]+", "", yanked_text)
        # Strip leading Nerd Font icons (common ranges)
        yanked_text = re.sub(r"^[\U0000E000-\U0000F8FF\U0001F300-\U0001F9FF\U00002600-\U000027BF\U00002300-\U000023FF]+", "", yanked_text)
        yanked_text = yanked_text.strip()
        if yanked_text:
            try:
                import subprocess
                import platform
                ...
                if cmd:
                    ...
                    proc.communicate(input=yanked_text.encode("utf-8"))
            except Exception:
                pass
        return ""
